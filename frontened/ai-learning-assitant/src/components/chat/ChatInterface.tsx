...
const ChatInterface = () => {
    ...
    useEffect(() => {
        const fetchChatHistory = async () => {
            try {
                ...
                console.error("Filed to fetch chat history:", error);
            } finally {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const assistantMessage = {
                ...
                timestamp: new Date(),
                relevantChunks: response.data.relevantChuncks,
            };
    ...
    const renderMessage = (msg, index) => {
        ...
        return (
            <div key={index} className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end': ''}`}>
                {!isUser && (
                    <div className="w-9 h-8 rounded-xl bg-linear-to-br from-emerald-400 to-teal-500 shadow-lg shadow-emerald-500/25 flex items-center justify-center shrink-0">
    ...
    return (
        <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
            {/* Messages Area */}
            <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 view-white/50 to-slate-50/50">
                {history.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                        ...
                        <h3 className="text-base font-semibold text-slate-900 mb-2">Start converstioan with AI</h3>
                    ...
                    <div className="flex items-center gap-3 my-4">
                        ...
                        <div className="flex items-center gap-2 px-4 py-3 rounded-2xl rounded-bl-md bg-white border border-slate-200/60">
                            <div className="flex gap-1">
                                ...
                                <span className="w-2 h-2 bg-slate-400 rounded-full animate-bounce" style={{ animationDely: '150ms'}}></span>
            ...
            <div className="p-5 border-t border-slate-200/60 bg-white/80">
                <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                    <input type="text"
                           ...
                           className="flex-1 h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placholder-slate-400 text-sm font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:bg-white focus:shadow-lg focus:shadow-emerald-500/10"
                           disabled={loading}
                           />
                        <button type="submit"
                                disabled={loading || !message.trim()}
                                className="shrink-0 h-12 w-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 flex items-center justify-center"
                                >
